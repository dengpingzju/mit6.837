...
PerspectiveCamera::PerspectiveCamera(Vec3f &center,Vec3f &direction,Vec3f &up,float angle) {
    ...
    if (h.Length()<0.5) {
		...
		//vector up and direction make a angle from away from 90 degrees
		cerr << "Warning: direction and up vectors are far from vertical position." << endl;
    }
    h.Normalize();
    ...
    usrScrOrigin=this->center+(dist*this->direction)-0.5*(this->up+ratio*this->horizontal);
    errCode=0;//all right
}
Ray PerspectiveCamera::generateRay(Vec2f point) {
    float x,y;
    point.Get(x,y);
    Vec3f scrPoint=this->usrScrOrigin+(x*ratio)*this->horizontal+y*this->up;
