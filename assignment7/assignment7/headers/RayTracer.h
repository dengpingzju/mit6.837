#ifndef _H_RAYTRACER_
#define _H_RAYTRACER_

#include "scene_parser.h"
#include "sampler.h"

class Filter;
class Film;

class RayTracer {
public:
	RayTracer(SceneParser *s, int max_bounces,
		float cutoff_weight, bool shadows);
	//detect if the shadow ray intersect with some object
	bool intersectShadowRay(const Ray &r, Hit &h, float tmin,float tmax) const;
	int shade(const Ray *ray, const Hit *hit, bool shadow, Vec3f &ret) const;
	
	static Vec3f mirrorDirection(const Vec3f &normal, const Vec3f &incoming);
	static bool transmittedDirection(const Vec3f &normal, const Vec3f &incoming,
		float index_i, float index_t, Vec3f &transmitted);
	Vec3f traceRay(Ray &ray, float tmin, int bounces, float weight,
		float indexOfRefraction, int role, Hit &hit,int accelerate = 0) const;
	void traceToImage(Image& img) const;
	void filterFilmToImage(Film *film,Image& img) const;
	void traceGridToImage(Image& img) const;
	Film* traceToFilm() const;

	void setGrid(Grid *g) {
		grid = g;
	}
	Grid* getGrid() const {
		return grid;
	}
	Sampler* getSampler() const {
		return sampler;
	}
	void setSampler(Sampler *s) {
		sampler = s;
	}
	Filter* getFilter() const {
		return filter;
	}
	void setFilter(Filter *f) {
		filter = f;
	}

	bool rayCast(const Ray &r, Hit &h, float tmin) const;
	bool rayCastFast(const Ray &r, Hit &h, float tmin) const;
	bool intersectWithPlanes(const Ray &r, Hit &h, float tmin) const;
private:
	SceneParser *pScene;
	int max_bounces;
	float cutoff_weight;
	bool shadows;

	Grid *grid;
	PobjVec planes;
	Sampler *sampler;
	Filter *filter;
};

#endif // !_H_RAYTRACER_

